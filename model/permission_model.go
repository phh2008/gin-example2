...
type PermissionModel struct {
	...
	PermName string `json:"permName" validate:"required"`             //权限名称
	Url      string `json:"url" validate:"required"`                  //URL路径
	Action   string `json:"action" validate:"required"`               //权限动作：比如get、post、delete等
	PermType uint8  `json:"permType" validate:"required,gte=1,lte=2"` //权限类型：1-菜单、2-按钮
